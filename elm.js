...
Elm.Api.make = function (_elm) {
   ...
   var currentMessages = $Signal.mailbox($Types.NoOp);
   var toAction = function (r) {
      return function () {
         switch (r.ctor)
         {case "Err": return $Types.NoOp;
            case "Ok":
            return $Types.Incoming(r._0);}
         _U.badCase($moduleName,
         "between lines 47 and 49");
      }();
   };
   var sendAction = $Signal.send(A2($Signal.forwardTo,
   currentMessages.address,
   toAction));
   var queryHandler = function (task) {
      return A2($Task.andThen,
      $Task.toResult(task),
      sendAction);
   };
   var messageDecoder = function () {
      var message = A3($Json$Decode.object2,
      F2(function (name,message) {
         return {_: {}
                ,message: message
                ,name: name};
      }),
      ...
      "message",
      $Json$Decode.string));
      return $Json$Decode.array(message);
   }();
   var getMessages = $Task.mapError($Basics.toString)(A2($Http.get,
   messageDecoder,
   "http://localhost:3000/messages"));
   var runQuery = Elm.Native.Task.make(_elm).performSignal("runQuery",
   $Signal.map(queryHandler)(A2($Signal.map,
   function (_v3) {
      return function () {
         return getMessages;
      }();
...
Elm.Types.make = function (_elm) {
   ...
   var NoOp = {ctor: "NoOp"};
   var Incoming = function (a) {
      return {ctor: "Incoming"
             ,_0: a};
   };
   ...
   var SendMessage = {ctor: "SendMessage"};
...
Elm.View.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm),
   $Types = Elm.Types.make(_elm);
   var messageList = function (model) {
      return function () {
         var msgRow = function (msg) {
            return $Html$Shorthand.tr_(_L.fromArray([$Html$Shorthand.td_(_L.fromArray([$Html$Shorthand.em_(msg.name)]))
         ...
         _L.fromArray([$Html$Shorthand.thead_(_L.fromArray([$Html$Shorthand.tr_(_L.fromArray([A2($Html.th,
                                                                                             ...
                                                                                             ,$Html$Shorthand.th_(_L.fromArray([$Html.text("Message")]))]))]))
                      ,$Html$Shorthand.tbody_($Array.toList(A2($Array.map,
                      msgRow,
                      model.messages)))]));
      }();
   };
   var sendMessage = function (key) {
      return _U.eq(key,
      13) ? $Types.SendMessage : $Types.NoOp;
   };
   var inputControls = F2(function (address,
   model) {
      return A2($Html.fieldset,
      _L.fromArray([]),
      _L.fromArray([$Html$Shorthand.legend_("Add Message")
                   ,A2($Html.form,
                   _L.fromArray([$Html$Attributes.$class("form-horizontal")
                                ,$Html$Attributes.action("#")]),
                   _L.fromArray([$Bootstrap$Html.formGroup_(_L.fromArray([A2($Html.label,
                                                                         _L.fromArray([$Html$Attributes.$for("name")
                                                                                      ,$Html$Attributes.$class("col-sm-1")]),
                                                                         _L.fromArray([$Html.text("Name")]))
                                                                         ,A2($Html.input,
                                                                         _L.fromArray([$Html$Attributes.id("name")
                                                                                      ,$Html$Attributes.$class("form-control")
                                                                                      ,$Html$Attributes.$class("col-sm-2")
                                                                                      ,$Html$Attributes.placeholder("Your Name")
                                                                                      ,A3($Html$Events.on,
                                                                                      "input",
                                                                                      $Html$Events.targetValue,
                                                                                      function ($) {
                                                                                         return $Signal.message(address)($Types.SetName($));
                                                                                      })]),
                                                                         _L.fromArray([]))]))
                                ,$Bootstrap$Html.formGroup_(_L.fromArray([A2($Html.label,
                                                                         _L.fromArray([$Html$Attributes.$for("say")
                                                                                      ,$Html$Attributes.$class("col-sm-1")]),
                                                                         _L.fromArray([$Html.text("Say")]))
                                                                         ,A2($Html.input,
                                                                         _L.fromArray([$Html$Attributes.id("say")
                                                                                      ,$Html$Attributes.$class("col-sm-9")
                                                                                      ...
                                                                                      $Html$Events.targetValue,
                                                                                      function ($) {
                                                                                         return $Signal.message(address)($Types.Input($));
                                                                                      })
                                                                                      ,A2($Html$Events.onKeyUp,
                                                                                      address,
                                                                                      sendMessage)]),
                                                                         _L.fromArray([]))]))
                                ,A3($Bootstrap$Html.btnPrimary_,
                                _U.replace([["label"
                                            ,$Maybe.Just("Send")]],
                                $Bootstrap$Html.btnParam),
                                address,
                                $Types.SendMessage)]))]));
   });
   var view = F2(function (address,
   model) {
      return $Bootstrap$Html.container_(_L.fromArray([A3($Html.node,
                                                     "link",
                                                     _L.fromArray([$Html$Attributes.rel("stylesheet")
                                                                  ,$Html$Attributes.href("https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css")]),
                                                     _L.fromArray([]))
                                                     ,A3($Html.node,
                                                     "link",
                                                     _L.fromArray([$Html$Attributes.rel("stylesheet")
                                                     ...
                                                     ,A3($Html.node,
                                                     "link",
                                                     _L.fromArray([$Html$Attributes.href("http://fonts.googleapis.com/css?family=Special+Elite")
                                                                  ,$Html$Attributes.rel("stylesheet")]),
                                                     _L.fromArray([]))
                                                     ,A2($Html.img,
                                                     _L.fromArray([$Html$Attributes.src("images/joan.png")]),
                                                     _L.fromArray([]))
                                                     ,$Html$Shorthand.h1_("Can We Talk!?")
                                                     ,$Bootstrap$Html.row_(_L.fromArray([messageList(model)]))
                                                     ,$Bootstrap$Html.row_(_L.fromArray([A2(inputControls,
                                                     address,
                                                     model)]))]));
   });
